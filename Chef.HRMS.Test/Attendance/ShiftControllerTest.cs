...
namespace Chef.HRMS.Test
{
    public class ShiftControllerTest : BaseTest
    {
        ...
        {
            //Arrange
            mockService.Setup(repo => repo.GetAllAsync()).Returns(Task.FromResult(GetMockShiftList()));
        ...
        public async void Add_ValidObjectPassed_ReturnedResponseHasCreatedItem()
        {
            ...
            mockService.Setup(service => service.InsertAsync(It.IsAny<Shift>())).Returns(Task.FromResult(GetMockShift()));
        ...
        public async void Remove_ExistingLeave_ReturnsOkResult()
        {
            // Arrange
            var existingId = 3;
            mockService.Setup(repo => repo.GetAsync(It.IsAny<int>())).Returns(Task.FromResult(GetMockShift()));
