...
@Directive({
  selector: '[smartInputRestriction]'
})
export class InputRestrictionDirective  {

  inputElement: ElementRef;

  @Input('smartInputRestriction') smartInputRestriction: string;
  arabicRegex = '[\u0600-\u06FF]';
  ...
  @HostListener('keypress', ['$event']) onKeyPress(event) {
    if (this.smartInputRestriction === 'integer') {
      this.integerOnly(event);
    } else if (this.smartInputRestriction === 'noSpecialChars') {
