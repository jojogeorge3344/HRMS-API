...
export class EmployeeExperienceDocumentsCreateComponent implements OnInit {
  ...
  onSubmit() {
    ...
    forkJoin([
      ...
      ([previousEmployment, document]) => {
        this.previousEmploymentDocument = {
          previousEmploymentId: previousEmployment,
          documentId: document,
        };
        this.previousEmploymentDocumentService
          .add(this.previousEmploymentDocument)
          .subscribe(
            (result: any) => {
              this.toastr.showSuccessMessage(
                "Previous Employment added successfully!"
              );
            ...
            (error) => {
              ...
              this.toastr.showErrorMessage(
                "There is an error in adding Previous Employment"
              );
            }
          );
      },
  ...
  setMaxDate() {
    this.maxDate = {
      year: new Date(
        this.addForm.controls["dateOfRelieving"].value
      ).getFullYear(),
      month:
        new Date(this.addForm.controls["dateOfRelieving"].value).getMonth() + 1,
      day: new Date(this.addForm.controls["dateOfRelieving"].value).getDate(),
    };
  }
  setMinDate() {
    this.minDate = {
      year: new Date(
        this.addForm.controls["dateOfJoining"].value
      ).getFullYear(),
      month:
        new Date(this.addForm.controls["dateOfJoining"].value).getMonth() + 1,
      day: new Date(this.addForm.controls["dateOfJoining"].value).getDate(),
    };
  }
  ...
  createFormGroup(): FormGroup {
    return this.formBuilder.group({
      ...
      jobTitle: ["", [Validators.required, Validators.maxLength(64)]],
      dateOfJoining: ["", [Validators.required]],
      dateOfRelieving: ["", [Validators.required]],
      location: ["", [Validators.required, Validators.maxLength(64)]],
      isApproved: [true],
