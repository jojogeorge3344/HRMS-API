...
export class EmployeeExperienceDocumentsEditComponent implements OnInit {
  ...
  removeFile() {
    ...
    documentPath.append("path", this.employmentDetails.path);
  ...
  onSubmit() {
    ...
    if (this.isFileChanged) {
      this.editForm.patchValue({
        document: { id: this.employmentDetails.documentId },
      });
      forkJoin([
        ...
        (result) => {
          this.toastr.showSuccessMessage(
            "Previous Employment updated successfully!"
          );
        ...
        (error) => {
          console.error(error);
          this.toastr.showErrorMessage(
            "There is an error in updating Previous Employment"
          );
        }
      );
    } else {
      this.previousEmploymentService.update(this.editForm.value).subscribe(
        (result: any) => {
          this.toastr.showSuccessMessage(
            "Previous Employment updated successfully!"
          );
  ...
  setMaxDate() {
    this.maxDate = {
      year: new Date(
        this.editForm.controls["dateOfRelieving"].value
      ).getFullYear(),
      month:
        new Date(this.editForm.controls["dateOfRelieving"].value).getMonth() +
        1,
      day: new Date(this.editForm.controls["dateOfRelieving"].value).getDate(),
    };
  }
  setMinDate() {
    this.minDate = {
      year: new Date(
        this.editForm.controls["dateOfJoining"].value
      ).getFullYear(),
      month:
        new Date(this.editForm.controls["dateOfJoining"].value).getMonth() + 1,
      day: new Date(this.editForm.controls["dateOfJoining"].value).getDate(),
    };
  }
  ...
  createFormGroup(): FormGroup {
    return this.formBuilder.group({
      ...
      companyName: [
        this.employmentDetails.companyName,
        [Validators.required, Validators.maxLength(64)],
      ],
      jobTitle: [
        this.employmentDetails.jobTitle,
        [Validators.required, Validators.maxLength(64)],
      ],
      dateOfJoining: [
        new Date(this.employmentDetails.dateOfJoining),
        [Validators.required],
      ],
      dateOfRelieving: [
        new Date(this.employmentDetails.dateOfRelieving),
        [Validators.required],
      ],
      location: [
        this.employmentDetails.location,
        [Validators.required, Validators.maxLength(64)],
      ],
      isApproved: [true],
