...
export class EmployeeJobDetailsViewComponent implements OnInit {
  ...
  getJobDetails() {
    forkJoin([
      this.jobDetailsService.get(this.currentUserId),
      this.employeeJobTitleService.getAll(),
      this.employeeService.getAll(),
    ]).subscribe(([details, titles, employees]) => {
      this.jobTitleName = titles.find(
        (title) => title.id === details.jobTitleId
      );
      this.jobTitles = titles;
      if (details.reportingManager && employees.length) {
        const emp = employees.find(
          (employee) => employee.id === details.reportingManager
        );
        if (emp)
          details.reportingTo = `${emp.firstName} ${emp.middleName || ""} ${
            emp.lastName
          }`;
      }
