...
export class EmployeeLeaveContainerComponent implements OnInit {
  ...
  getLeaveSettings() {
    this.employeeLeaveService.getAllLeaveSettings(this.currentUserId).subscribe(result => {
  ...
  leaveCriteria() {
    ...
    if (this.leaveSettings.canEmployeeApplyLeave) {
      if (!this.leaveSettings.canApplyLeaveDuringProbation) {
        if (this.leaveSettings.periodType == 1 && diffDays < this.leaveSettings.probationPeriod) {
          return true;
        } else if (this.leaveSettings.periodType == 2 && (diffDays / 7) < this.leaveSettings.probationPeriod) {
          return true;
        } else {
          return false;
        }
