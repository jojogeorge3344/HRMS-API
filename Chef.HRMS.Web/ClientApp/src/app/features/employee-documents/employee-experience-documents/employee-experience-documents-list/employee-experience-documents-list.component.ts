...
export class EmployeeExperienceDocumentsListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.getPreviousEmployment(this.currentUserId);
  }

  getPreviousEmployment(employeeId: number) {
    this.previousEmploymentService.get(this.currentUserId).subscribe((result: any) => {
      if (result.length) {
  ...
  openAdd() {
    const modalRef = this.modalService.open(EmployeeExperienceDocumentsCreateComponent,
  ...
  openEditPreviousEmployment(previousEmployment: EmployeeExperienceDetails) {
    const modalRef = this.modalService.open(EmployeeExperienceDocumentsEditComponent,
      { size: 'lg', centered: true, backdrop: 'static' });

    modalRef.componentInstance.previousEmployment = previousEmployment;
  ...
  deletePreviousEmployment(previousEmployment: any) {
    ...
    modalRef.result.then((userResponse) => {
      if (userResponse == true) {
        forkJoin([
          this.previousEmploymentService.delete(previousEmployment.previousEmploymentId),
