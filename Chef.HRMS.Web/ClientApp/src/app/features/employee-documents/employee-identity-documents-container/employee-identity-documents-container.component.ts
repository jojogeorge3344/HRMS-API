...
export class EmployeeIdentityDocumentsContainerComponent implements OnInit {
  ...
  openAdd() {
    const modalRef = this.modalService.open(EmployeeIdentityDocumentsCreateComponent,
  ...
  getIdentityDetails() {
  debugger    
    this.identityDetailsService.getEmployeeId(this.currentUserId).subscribe((result: any) => {      
      if (result.length) {
        this.identityDetails = result;
        this.identityDetails.forEach((element) => {
          element.documentType = this.getDocumentTypeValue(element.documentTypeList);
        });
        this.isEmpty = false;
  ...
  openEditEducationalDetails(identityDetails: EmployeeIdentityDetails) {
    const modalRef = this.modalService.open(EmployeeIdentityDocumentsEditComponent,
      { size: 'lg', centered: true, backdrop: 'static' });
  ...
  this.identityDetailsService.getAllByEmployeeId(this.currentUserId,document.documentId).subscribe((result)=>{
    debugger
    console.log("response",result)
  var url = result[0].path
  var sanitizedurl =this.sanitizer.bypassSecurityTrustResourceUrl('https://upload.wikimedia.org/wikipedia/commons/thumb/b/b6/Image_created_with_a_mobile_phone.png/640px-Image_created_with_a_mobile_phone.png')
