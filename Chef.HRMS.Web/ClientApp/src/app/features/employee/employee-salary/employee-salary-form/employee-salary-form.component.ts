...
export class EmployeeSalaryFormComponent implements OnInit {
  ...
  @Input() salaryStructure: EmployeeSalaryConfigurationView[];
  ...
  setupForm() {
    ...
    (this.salaryForm.get('salaryArray') as FormArray).controls.forEach(
      control => {
        control.get('yearly').valueChanges.subscribe(
          (res) => {
            const index = (this.salaryForm.get('salaryArray') as FormArray).controls.indexOf(control);
            if (res !== this.salaryStructure[index].yearlyAmount) {
              ((this.salaryForm.get('salaryArray') as FormArray).at(index) as FormGroup)
                .get('monthly').patchValue(parseFloat((res / 12).toFixed(2)), { emitEvent: false });
              ((this.salaryForm.get('salaryArray') as FormArray).at(index) as FormGroup)
                .get('monthly').markAsDirty();
            }
          }
        );
        control.get('monthly').valueChanges.subscribe(
          (res) => {
            const index = (this.salaryForm.get('salaryArray') as FormArray).controls.indexOf(control);
            if (res !== this.salaryStructure[index].monthlyAmount) {
              ((this.salaryForm.get('salaryArray') as FormArray).at(index) as FormGroup)
                .get('yearly').patchValue(parseFloat((res * 12).toFixed(2)), { emitEvent: false });
              ((this.salaryForm.get('salaryArray') as FormArray).at(index) as FormGroup)
                .get('yearly').markAsDirty();
            }
          }
        );
      }
  ...
  calculateTotal() {
    this.salaryTotalMonthly = this.salaryStructure.reduce((sum, x) => sum + x.monthlyAmount, 0);
    this.salaryTotalYearly = this.salaryStructure.reduce((sum, x) => sum + x.yearlyAmount, 0);  
  }
  ...
  setupControls() {
    this.salaryForm.controls.salaryArray.valueChanges.subscribe(res => {
      ...
      this.salaryStructure = this.salaryStructure.map((comp, i) => { 
        this.valueObject[`[${comp.shortCode}]`] = comp.isComputed ? comp.monthlyAmount : res[i].monthly;
        return {
          ...comp,
      ...
      this.salaryForm.getRawValue().salaryArray.map((control, i) => {
        monthly += this.salaryStructure[i].formula ? this.salaryStructure[i].monthlyAmount : control.monthly;
        yearly += this.salaryStructure[i].formula ? this.salaryStructure[i].yearlyAmount : control.yearly;
      });
    ...
    this.salaryStructure.map((res, i) => {
      ...
      if (res.formula) {
        ...
        ((this.salaryForm.get('salaryArray') as FormArray).at(i) as FormGroup)
          .get('monthly').patchValue(parseFloat((res.monthlyAmount).toFixed(2)), { emitEvent: false });
        this.setControl(((this.salaryForm.get('salaryArray') as FormArray).at(i) as FormGroup)
          .get('monthly'), res.maximumLimit);
        this.valueObject[`[${res.shortCode}]`] = res.monthlyAmount;
        res.yearlyAmount = eval(formula) * 12;
        ((this.salaryForm.get('salaryArray') as FormArray).at(i) as FormGroup)
          .get('yearly').patchValue(parseFloat((res.yearlyAmount).toFixed(2)), { emitEvent: false });
        this.setControl(((this.salaryForm.get('salaryArray') as FormArray).at(i) as FormGroup)
          .get('yearly'), res.maximumLimit * 12);
      }
    });
  }
}
