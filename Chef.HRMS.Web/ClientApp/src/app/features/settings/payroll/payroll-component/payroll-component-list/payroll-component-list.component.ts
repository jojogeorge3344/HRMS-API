...
export class PayrollComponentListComponent implements OnInit {
  ...
  isDisabled(payrollComponent) {
    return (this.isFixed(payrollComponent.payrollComponentType) || this.assignedPayrollComponents.includes(payrollComponent.id));
  }
  ...
  isFixed(payrollComponent) {
    return payrollComponent.payrollComponentType == this.payrollComponentTypes.Fixed;
  }
  ...
  openCreatePayrollComponent() {
    ...
    modalRef.componentInstance.payrollComponentCodes = this.payrollComponentCodes;
  ...
  openEditPayrollComponent(payrollComponent: PayrollComponent) {
    ...
    modalRef.componentInstance.payrollComponentNames = this.payrollComponentNames.filter(v => v !== payrollComponent.name.toLowerCase());
    modalRef.componentInstance.payrollComponentCodes = this.payrollComponentCodes.filter(v => v !== payrollComponent.shortCode.toLowerCase());
  ...
  deletePayrollComponent(payrollComponent: PayrollComponent) {
    ...
    modalRef.result.then((userResponse) => {
      if (userResponse == true) {
        this.payrollComponentService.delete(payrollComponent.id).subscribe(() => {
          this.toastr.showSuccessMessage('The payroll component is deleted successfully!');
          this.getPayrollComponents();
        });
      }
    });
