...
export class PayrollCalculationEditComponent implements OnInit {
  ...
  getSystemVarible() {
    this.payrollsettingsService.getAll().subscribe(result => {
      ...
      error => {
        console.error(error);
        this.toastr.showErrorMessage('Unable to fetch the Payroll System Variables');
      });
  }
  ...
  onSubmit() {
    ...
    if (assignCalculationOvertimeValue.id == 0 || assignCalculationOvertimeValue.id == null) {
      this.payrollCalculationService.add(assignCalculationOvertimeValue).subscribe(result => {
        this.toastr.showSuccessMessage('Payroll Formula added successfully');
      ...
      this.payrollCalculationService.update(assignCalculationOvertimeValue).subscribe(result => {
        this.toastr.showSuccessMessage('Payroll Formula updated successfully');
        this.activeModal.close('submit');
      },
        error => {
          console.error(error);
          this.toastr.showErrorMessage('Unable to update the Payroll Formula');
        });
    }
